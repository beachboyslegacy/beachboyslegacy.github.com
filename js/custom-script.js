            ...
            $thisItem = '<li id="'+data.items[i].parent.uniqueId+'" data-rating="'+data.items[i].parent.aggregateRating+'" data-date="'+data.items[i].parent.releaseYear+'">'+
                              ...
                              '<div class="item__body">'+
                                '<div class="parent-item__info">'+
                                  ...
                                  '<p>'+data.items[i].parent.publisher+' ('+data.items[i].parent.country+') '+data.items[i].parent.code+'</p>'+
              ...
              for (k=0; k<tracks; k++){
                ...
                if( data.items[i].parent.track.itemListElement[k].item.hit == true ){
                  thisTrack = '<tr>'+
                                    '<td><div class="tracknumber">'+data.items[i].parent.track.itemListElement[k].item.track+'</div>'+data.items[i].parent.track.itemListElement[k].item.name+'</td>'+
        ...
        for(i=0; i<totalItems; i++){
          ...
          if( (data.items[i].parent.byArtist === selectedArtist) && (data.items[i].parent.category[selectedCategory] == true) ){
            ...
            if( (data.items[i].parent.category.video) && !(data.items[i].parent.category.live) ){
              $thisItem = '<li id="'+data.items[i].parent.uniqueId+'" data-rating="'+data.items[i].parent.aggregateRating+'" data-date="'+data.items[i].parent.releaseYear+'">'+
                                ...
                                '<div class="item__body">'+
                                  '<div class="parent-item__info">'+
                                    '<p>Released on '+thisReleaseMonthName+' '+data.items[i].parent.releaseDay+', '+data.items[i].parent.releaseYear+'</p>'+
                                    '<p>'+data.items[i].parent.publisher+' ('+data.items[i].parent.country+')</p>'+
                                    // '<p>'+data.items[i].parent.notes+'</p>'+
                                  '</div>'+
                                  '<ul class="child-items"></ul>'+
                                  '<div class="item__footer">'+
              ...
              $thisItem = '<li id="'+data.items[i].parent.uniqueId+'" data-rating="'+data.items[i].parent.aggregateRating+'" data-date="'+data.items[i].parent.releaseYear+'">'+
                                ...
                                '<div class="item__body">'+
                                  '<div class="parent-item__info">'+
                                    '<p>Released on '+thisReleaseMonthName+' '+data.items[i].parent.releaseDay+', '+data.items[i].parent.releaseYear+'</p>'+
                                    '<p>'+data.items[i].parent.publisher+' ('+data.items[i].parent.country+')</p>'+
                                    '<p> ISBN: '+data.items[i].parent.isbn+'</p>'+
                                    // '<p>'+data.items[i].parent.notes+'</p>'+
                                  '</div>'+
                                  '<ul class="child-items"></ul>'+
                                  '<div class="item__footer">'+
            ...
            // ALBUM
              ...
              // Tracklist is loaded if the item is not a book
              if(!data.items[i].parent.category.book){
                tracks = data.items[i].parent.track.itemListElement.length;
        ...
        if ( thisButton.hasClass('visited') ){
          ...
          for (j=childrenNumber-1; j>=0; j--){
            ...
            if(data.items[requestedItemNumber].parent.category.book){
              // Book Children don't have a tracklist
              var $thisChild = '<li>'+
                                ...
                                '</div>'+
                              '</li>';
            // All items but books have a tracklist
            }else{
              var $thisChild = '<li>'+
                                '<div class="child-item__header">'+
                                  '<h3 class="child-item__title">'+data.items[requestedItemNumber].editions[j].name+'</h3>'+
                                ...
                                '<div class="child-item__body">'+
                                  ...
                                  '<div class="child-item__tracklist">'+
                                    '<pre>'+data.items[requestedItemNumber].editions[j].track+'</pre>'+
                                  '</div>'+
                                '</div>'+
                              '</li>';
            }
